...
// 设置当前是否为测试模式，开发者在开发过程中可以使用该模式向服务器请求测试广告。
// 返回值:YES表示测试模式，NO表示非测试模式。
- (BOOL)domobIsTestingMode {
	return NO;
}

/*
// 设置测试模式下，点击后的广告展现类型， 类型包括 :url, map, market, call, sms, mail, video, audio，默认类型为”url”。
// 返回值:点击广告展现类型的字符串，例如：”url”。
- (NSString *)domobTestAction:(DoMobView *)doMobView {
	return @"url";
}
 */
